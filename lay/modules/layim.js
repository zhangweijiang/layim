...
layui.define(["layer", "laytpl", "upload"],
    function(i) {
        ...
        var u = function(i) {
            ...
            h = ['<li {{ d.mine ? "class=layim-chat-mine" : "class=layim-chat-others" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }} {{# if(d.userId){ }}data-userId="{{d.userId}}"{{# } }} {{# if(d.userLevel!==null ){ }}data-userlevel="{{d.userLevel}}"{{# } }} >', '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}", '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', "</li>"].join(""),
            v = '<li class="layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(true){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>',
            g = function(i) {
                return i < 10 ? "0" + (0 | i) : i
            };
        ...
        var x, b, w, k, C, S = function(i, a, n) {
            ...
            B = function(i) {
                //return;
                //console.log(i);
                if(!i.hasOwnProperty("response") || !i["response"]) {
                    return;
                }
                var a = layui.data("layim")[j.mine.id] || {};
                a.chatlog = a.chatlog || {};
           ...
           /* Q = function() {
                var i = ["[加1]", "[666]", "[NO]", "[OK]", "[跌停板]", "[发财啦]", "[佛系炒股]", "[火箭]", "[接盘侠]", "[开盘啦]", "[买买买]", "[求个牛股]", "[求内幕]", "[生气]", "[送花]", "[套牢]", "[小赚一笔]", "[心好痛]", "[赞]", "[涨疯了]", "[涨停板]", "[终于解套]"],
                    a = {};
                return layui.each(i,
                    function(i, e) {
                        a[e] = layui.cache.dir + "emotion/emotion_" + (i + 1) + ".png"
                    }),
                    a
            } (),*/
